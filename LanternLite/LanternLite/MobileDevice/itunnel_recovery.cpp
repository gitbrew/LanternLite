...
void dfu_connect_callback(AMRecoveryModeDevice device) 
{
	...
	if (*g_ibss && (g_icmdState == ICMD_ZERO)) {
		int result = libmd_builtin_uploadFileDfu(device, g_ibss);
		Log(LOG_INFO, "iBSS %s loaded: %u", g_ibss, result);
    g_icmdState = ICMD_SENT_IBSS;
	}
	if (*g_ibec && (g_icmdState == ICMD_ZERO)) {
		int result = uploadFile(device, g_ibec);
		Log(LOG_INFO, "iBEC %s loaded: %u", g_ibec, result);
    Sleep(1000);
    result = libmd_builtin_sendCommand(device, g_goCmd);
    Log(LOG_INFO, "go command sent: %u", result);
